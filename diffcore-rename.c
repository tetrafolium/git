...
static int estimate_similarity(struct repository *r, struct diff_filespec *src,
			       ...
			       int skip_unmodified)
{
	...
	unsigned long max_size, delta_size, base_size, src_copied,
		literal_added;
	int score;
	struct diff_populate_filespec_options dpf_options = { .check_size_only =
								      1 };
...
static void insert_file_table(struct repository *r, struct hashmap *table,
			      int index, struct diff_filespec *filespec)
{
...
static int find_exact_renames(struct diff_options *options)
{
	...
	for (i = rename_src_nr - 1; i >= 0; i--)
		insert_file_table(options->repo, &file_table, i,
				  rename_src[i].p->one);
...
static int find_renames(struct diff_score *mx, int dst_cnt, int minimum_score,
			int copies)
{
	int count = 0, i;
...
void diffcore_rename(struct diff_options *options)
{
	...
	if (want_copies || break_idx) {
		...
		rename_count +=
			find_basename_matches(options, min_basename_score);
	...
	for (dst_cnt = i = 0; i < rename_dst_nr; i++) {
		...
		for (j = 0; j < rename_src_nr; j++) {
			...
			this_src.score = estimate_similarity(options->repo, one,
							     two, minimum_score,
							     skip_unmodified);
...
cleanup:
	...
	for (i = 0; i < q->nr; i++) {
		...
		if (DIFF_PAIR_UNMERGED(p)) {
			...
			/* no need to keep unmodified pairs; FIXME: remove
			 * earlier? */
			pair_to_free = p;
