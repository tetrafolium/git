 ...
 *
 * @param {String} sName:    Unique name of a cookie (letters, numbers,
 *     underscores).
 ...
 * @param {String|Number|Date} [options.expires] Either literal string to be
 *     used as cookie expires,
 *                            or an integer specifying the expiration date from
 * now on in days, or a Date object to be used as cookie expiration date. If a
 * negative value is specified or a date in the past), the cookie will be
 * deleted. If set to null or omitted, the cookie will be a session cookie and
 * will not be retained when the browser exits.
 * @param {String} [options.path] Restrict access of a cookie to particular
 *     directory
 *                               (default: path of page that created the
 * cookie).
 * @param {String} [options.domain] Override what web sites are allowed to
 *     access cookie
 *                                  (default: domain of page that created the
 * cookie).
 * @param {Boolean} [options.secure] If true, the secure attribute of the cookie
 *     will be set
 *                                   and the cookie would be accessible only
 * from secure sites (cookie transmission will require secure protocol like
 * HTTPS).
 */
    ...
    if (oExpires === 'delete') {
      ...
      if (typeof oExpires === 'number') {
        ...
        oDate.setTime(oDate.getTime() +
                      (oExpires * 24 * 60 * 60 * 1000)); // days to ms
      } else {
        oDate = oExpires;
      }
