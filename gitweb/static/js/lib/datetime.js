 ...
 *
 * @param {String} date: date in RFC-2822 format, e.g. 'Thu, 21 Dec 2000
 *     16:01:07 +0200'
 * @returns {Number} epoch i.e. seconds since '00:00:00 1970-01-01 UTC'
 */
function parseRFC2822Date(date) {
  // Date.parse accepts the IETF standard (RFC 1123 Section 5.2.14 and
  // elsewhere) date syntax, which is defined in RFC 2822 (obsoletes RFC 822)
...
function formatTimezoneInfo(hours, minutes, sep) {
  ...
  if (minutes < 0 || minutes > 59) {
    ...
    minutes =
        Math.abs(minutes - 60 * hours); // sign of minutes is sign of hours
...
function formatDateRFC2882(epoch, timezoneInfo, padDay) {
  ...
  var localDateStr = // e.g. 'Sun, 7 Aug 2005' or 'Sun, 07 Aug 2005'
      ...
      (padDay ? padLeft(localDate.getUTCDate(), 2, '0')
              : localDate.getUTCDate()) +
      ' ' + months[localDate.getUTCMonth()] + ' ' + localDate.getUTCFullYear();
  var localTimeStr = // e.g. '21:49:46'
      padLeft(localDate.getUTCHours(), 2, '0') + ':' +
      padLeft(localDate.getUTCMinutes(), 2, '0') + ':' +
      padLeft(localDate.getUTCSeconds(), 2, '0');
