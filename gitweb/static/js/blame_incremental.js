 ...
 */
function isStartOfGroup(tr) { return tr.firstChild.className === 'sha1'; }
...
function fixColorsAndGroups() {
  ...
  while ((tr = document.getElementById('l' + linenum))) {
    ...
    if (isStartOfGroup(tr, linenum, document)) {
      if (prev_group && prev_group.firstChild.firstChild.href ===
...
function handleLine(commit, group) {
  ...
  */
  ...
  var colorNo =
      findColorNo(document.getElementById('l' + (resline - 1)),
  ...
  for (var i = 0; i < group.numlines; i++, resline++) {
    ...
    */
    ...
    a_linenr.href = projectUrl + 'a=blame_incremental' +
                    ';hb=' + linenr_commit +
                    ';f=' + encodeURIComponent(linenr_filename) + '#l' +
                    (group.srcline + i);
...
function handleError(xhr) {
  errorInfo('Server error: ' + xhr.status + ' - ' +
...
function handleResponse(xhr, fromTimer) {
  ...
  if (fromTimer) {
    setTimeout(function() { handleResponse(xhr, true); }, 1000);
...
function startBlame(blamedataUrl, bUrl) {
  ...
  xhr.onreadystatechange = function() { handleResponse(xhr, false); };
  ...
  xhr.pollTimer = setTimeout(function() { handleResponse(xhr, true); }, 1000);
}
