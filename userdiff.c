...
static struct userdiff_driver builtin_drivers[] = {
	...
	PATTERNS("dts",
		 "!;\n"
		 "!=\n"
		 /* lines beginning with a word optionally preceded by '&' or
		    the root */
		 "^[ \t]*((/[ \t]*\\{|&?[a-zA-Z_]).*)",
		 /* -- */
	...
	PATTERNS(
		"matlab",
		/*
		 * Octave pattern is mostly the same as matlab, except that
		 * '%%%' and
...
static struct userdiff_driver driver_true = { "diff=true",
					      NULL,
					      0,
					      { NULL, 0 } };
