...
#define PATHSPEC_ALL_MAGIC                                         \
	(PATHSPEC_FROMTOP | PATHSPEC_MAXDEPTH | PATHSPEC_LITERAL | \
	 PATHSPEC_GLOB | PATHSPEC_ICASE | PATHSPEC_EXCLUDE | PATHSPEC_ATTR)
...
#define GUARD_PATHSPEC(ps, mask)                                         \
	do {                                                             \
		if ((ps)->magic & ~(mask))                               \
			die("BUG:%s:%d: unsupported magic %x", __FILE__, \
			    __LINE__, (ps)->magic & ~(mask));            \
	} while (0)
...
static inline int ps_strncmp(const struct pathspec_item *item, const char *s1,
...
static inline int ps_strcmp(const struct pathspec_item *item, const char *s1,
			    const char *s2)
{
	if (item->magic & PATHSPEC_ICASE)
