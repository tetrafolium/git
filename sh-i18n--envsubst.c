...
int cmd_main(int argc, const char *argv[])
{
	...
	case 1:
		...
		*/
	case 2:
		/* echo '$foo and $bar' | git sh-i18n--envsubst --variables
		 * '$foo and $bar' */
		all_variables = 0;
	...
	/* Close standard error.  This is simpler than fwriteerror_no_ebadf,
	   because upon failure we don't need an errno - all we can do at this
	   point is to set an exit status.  */
	errno = 0;
...
static void find_variables(const char *string,
			   void (*callback)(const char *var_ptr,
					    size_t var_len))
{
	for (; *string != '\0';)
		if (*string++ == '$') {
...
static void print_variable(const char *var_ptr, size_t var_len)
{
...
static inline void string_list_init(string_list_ty *slp)
{
	slp->item = NULL;
	slp->nitems = 0;
	slp->nitems_max = 0;
...
static inline void string_list_append(string_list_ty *slp, const char *s)
{
	/* Grow the list.  */
...
static int cmp_string(const void *pstr1, const void *pstr2)
{
...
static inline void string_list_sort(string_list_ty *slp)
{
	QSORT(slp->item, slp->nitems, cmp_string);
}

/* Test whether a sorted string list contains a given string.  */
static int sorted_string_list_member(const string_list_ty *slp, const char *s)
{
	size_t j1, j2;
	...
	if (j2 > 0) {
		/* Binary search.  */
		while (j2 - j1 > 1) {
			/* Here we know that if s is in the list, it is at an
			   index j with j1 <= j < j2.  */
			size_t j = j1 + ((j2 - j1) >> 1);
...
static void note_variable(const char *var_ptr, size_t var_len)
{
...
static inline void do_ungetc(int c)
{
	if (c != EOF)
		ungetc(c, stdin);
}
	...
	for (;;) {
		...
		if (c == '$') {
			...
			if ((c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z') ||
				...
				if (valid) {
					/* Substitute the variable's value from
					 * the environment.  */
					...
					/* Perform no substitution at all. Since
					   the buffered input contains no other
					   '$' than at the start, we can just
