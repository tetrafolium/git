				...
				struct setup_revision_opt *opt)
{
	...
	struct decoration_filter decoration_filter = {
		&decorate_refs_include, &decorate_refs_exclude,
...
static void make_cover_letter(struct rev_info *rev, int use_separate_file,
			      ...
			      struct commit **list, const char *branch_name,
			      int quiet)
{
				      ...
				      struct commit **list, int total)
{
	...
	while (rev_nr > 1) {
		for (i = 0; i < rev_nr / 2; i++) {
			...
			merge_base =
				get_merge_bases(rev[2 * i], rev[2 * i + 1]);
...
int cmd_format_patch(int argc, const char **argv, const char *prefix)
{
	...
	const struct option builtin_format_patch_options[] = {
		...
		OPT_CALLBACK(0, "to", NULL, N_("email"), N_("add To: header"),
			     to_callback),
		OPT_CALLBACK(0, "cc", NULL, N_("email"), N_("add Cc: header"),
			     cc_callback),
		OPT_CALLBACK_F(
	...
	if (0 < reroll_count) {
		...
		strbuf_addf(&sprefix, "%s v%d", rev.subject_prefix,
			    reroll_count);
