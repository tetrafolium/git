...
static const char *const builtin_add_usage[] = {
	N_("git add [<options>] [--] <pathspec>..."), NULL
};
			    ...
			    struct diff_options *opt, void *cbdata)
{
	...
	for (i = 0; i < q->nr; i++) {
		...
		case DIFF_STATUS_DELETED:
			...
			if (data->flags &
			    (ADD_CACHE_PRETEND | ADD_CACHE_VERBOSE))
				printf(_("remove '%s'\n"), path);
			break;
		}
	}
...
static int renormalize_tracked_files(const struct pathspec *pathspec, int flags)
{
	...
	for (i = 0; i < active_nr; i++) {
		...
		if (pathspec && !ce_path_match(&the_index, ce, pathspec, NULL))
			continue;
		retval |= add_file_to_cache(ce->name,
...
static void refresh(int verbose, const struct pathspec *pathspec)
{
	...
	refresh_index(&the_index,
		      verbose ? REFRESH_IN_PORCELAIN : REFRESH_QUIET, pathspec,
		      seen, _("Unstaged changes after refreshing the index:"));
			...
			const struct pathspec *pathspec)
{
	...
	int use_builtin_add_i = git_env_bool("GIT_TEST_ADD_I_USE_BUILTIN", -1);
...
int interactive_add(const char **argv, const char *prefix, int patch)
{
	...
	return run_add_interactive(NULL, patch ? "--patch" : NULL, &pathspec);
}

static int edit_patch(int argc, const char **argv, const char *prefix)
{
	char *file = git_pathdup("ADD_EDIT.patch");
	const char *apply_argv[] = { "apply", "--recount", "--cached", NULL,
				     NULL };
...
static const char ignore_error[] = N_(
...
static int ignore_removal_cb(const struct option *opt, const char *arg,
			     int unset)
{
...
static struct option builtin_add_options[] = {
	...
	OPT_BOOL(0, "refresh", &refresh_only,
		 N_("don't add, only refresh the index")),
	OPT_BOOL(0, "ignore-errors", &ignore_add_errors,
