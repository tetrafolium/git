...
#define REV_SHIFT 2
#define MAX_REVS                                                              \
	(FLAG_BITS - REV_SHIFT) /* should not exceed bits_per_int - REV_SHIFT \
				 */
...
static void show_one_commit(struct commit *commit, int no_name)
{
	...
	if (!no_name) {
		if (name && name->head_name) {
			...
			printf("[%s] ", find_unique_abbrev(&commit->object.oid,
							   DEFAULT_ABBREV));
	}
	puts(pretty_str);
...
int cmd_show_branch(int ac, const char **av, const char *prefix)
{
	...
	struct option builtin_show_branch_options[] = {
		...
		OPT_CALLBACK_F(
			'g', "reflog", &reflog_base, N_("<n>[,<base>]"),
			N_("show <n> most recent ref-log entries starting at "
			   "base"),
			PARSE_OPT_OPTARG | PARSE_OPT_NONEG, parse_reflog_param),
		OPT_END()
	};
	...
	if (reflog) {
		...
		if (ac == 0) {
			...
			fake_av[0] = resolve_refdup("HEAD", RESOLVE_REF_READING,
						    &oid, NULL);
			fake_av[1] = NULL;
			av = fake_av;
			ac = 1;
		...
		for (i = 0; i < reflog; i++) {
			...
			if (read_ref_at(get_main_ref_store(the_repository), ref,
					flags, 0, base + i, &oid, &logmsg,
					&timestamp, &tz, NULL)) {
				reflog = i;
				break;
			}
	...
	if (with_current_branch && head) {
		...
		for (i = 0; !has_head && i < ref_name_cnt; i++) {
			 ...
			 * HEAD points at.
			 */
			if (rev_is_head(head, ref_name[i], head_oid.hash, NULL))
				has_head++;
		}
		if (!has_head) {
	...
	for (num_rev = 0; ref_name[num_rev]; num_rev++) {
		...
		if (MAX_REVS <= num_rev)
			die(Q_("cannot handle more than %d rev.",
			       "cannot handle more than %d revs.", MAX_REVS),
			    MAX_REVS);
		if (get_oid(ref_name[num_rev], &revkey))
			die(_("'%s' is not a valid ref."), ref_name[num_rev]);
		commit = lookup_commit_reference(the_repository, &revkey);
		if (!commit)
			die(_("cannot find commit %s (%s)"), ref_name[num_rev],
			    oid_to_hex(&revkey));
		parse_commit(commit);
	...
	if (1 < num_rev || extra < 0) {
		for (i = 0; i < num_rev; i++) {
			...
			if (extra < 0)
				...
				for (j = 0; j < i; j++)
					putchar(' ');
				printf("%s%c%s [%s] ", get_color_code(i),
				       is_head ? '*' : '!',
		...
		if (1 < num_rev) {
			...
			for (i = 0; i < num_rev; i++) {
				...
				if (!(this_flag & (1u << (i + REV_SHIFT))))
					...
					mark = '+';
				printf("%s%c%s", get_color_code(i), mark,
