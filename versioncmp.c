 ...
 */
static int swap_prereleases(const char *s1, const char *s2, int off, int *diff)
{
	int i;
	...
	for (i = 0; i < prereleases->nr; i++) {
		...
		if (suffix_len < off)
			...
			start = 0;
		find_better_matching_suffix(s1, suffix, suffix_len, start, i,
					    &match1);
		find_better_matching_suffix(s2, suffix, suffix_len, start, i,
					    &match2);
	}
...
int versioncmp(const char *s1, const char *s2)
{
	...
	if (!initialized) {
		...
		deprecated_prereleases = git_config_get_value_multi(
