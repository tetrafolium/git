...
#define bitset_clear(set, i)          \
	(set[i / BITSET_WORD_BITS] &= \
	 ~((bitset_word_t)1 << i % BITSET_WORD_BITS))
...
typedef struct {
	...
	/* These 2 bits can be moved into the union if needed (e.g. if running
	   out of bits; move opr.c to opr.c.c and move the flags to
	   opr.c.flags).  */
...
struct re_string_t {
	...
	/* The translation passed as a part of an argument of
	 * re_compile_pattern.  */
...
#define NOT_SATISFY_PREV_CONSTRAINT(constraint, context)                       \
	((((constraint)&PREV_WORD_CONSTRAINT) && !IS_WORD_CONTEXT(context)) || \
	 ((constraint & PREV_NOTWORD_CONSTRAINT) &&                            \
